...
export class AuthController {
  ...
  async googleLogin(@Body() dto: GoogleLoginDto, @Res({ passthrough: true }) res: Response) {
    const { accessToken, refreshToken, user } = await this.authService.googleLogin(dto.idToken);
  ...
  async refresh(@CurrentUser() user: JwtPayload, @Res({ passthrough: true }) res: Response) {
    const { accessToken, refreshToken, user: payload } = await this.authService.refreshTokens(
      user.sub,
    );
    res.cookie('refresh_token', refreshToken, REFRESH_COOKIE_OPTIONS);
